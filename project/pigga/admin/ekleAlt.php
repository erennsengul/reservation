...
<div id="wrapper">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div class="content my-5 p-lg-5">
            <label for="exampleInputEmail1"><h4>Alt Yemek Ekle</h4></label><hr>
            <form method="post" action="ekleAlt.php">
                <div class="mb-3">
                    <label for="exampleInputEmail1" class="form-label">Yemek Adı</label>
                    <input type="text" class="form-control" name="yemekAdi" id="exampleInputEmail1" aria-describedby="emailHelp">
                </div>
                <div class="mb-3">
                    <label for="exampleInputPassword1" class="form-label">Yemek Fiyatı</label>
                    <input type="text" class="form-control" name="yemekFiyat" id="exampleInputPassword1">
                </div>
            ...
            if ($_POST){
                if (!empty($_POST["yemekAdi"]) and !empty($_POST["yemekFiyat"]) and !empty($_POST["check"])) {
                        $YemekAdiAl = $_POST["yemekAdi"];
                        $YemekFiyatAl = $_POST["yemekFiyat"];
                        $Check = $_POST['check'];

                        $ekle = $connect->prepare("INSERT INTO alt_yemekler(yemekAdi,yemekFiyat) VALUES (?,?)");
                        $ekle->execute(array(
                            $YemekAdiAl,$YemekFiyatAl
                        ));
